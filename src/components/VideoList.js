	...
	renderVideo(video) {
		return (
			<List animated verticalAlign="middle" key={video.etag}>
				<List.Item onClick={() => this.props.reproduzVideo(video)}>
					<Image rounded size="small" src={video.snippet.thumbnails.default.url} />
					<List.Content>
						<List.Header>
							{video.snippet.title}
						</List.Header>
...
const mapDispatchToProps = (dispatch) => {
	return {
		reproduzVideo: (video) => dispatch(reproduzVideo(video))
	}
}
